...
fn main() -> io::Result<()> {
    ...
    let buff_writer = BufWriter::new(write_clone);
    let buff_reader = BufReader::new(read_clone);

    let mut wal = Wal::new(buff_writer, buff_reader);

    wal.write_entry("test".as_bytes(), "test".as_bytes(), 0x01)?;
